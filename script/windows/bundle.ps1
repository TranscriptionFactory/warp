...
if ("$CHANNEL" -eq 'local') {
    ...
    $WARP_BIN = 'openwarp-oss'
    $BINARY_NAME = 'openwarp-oss.exe'
    $APP_NAME = 'OpenWarp'
...
# 静默吞掉 toast。OSS(OpenWarp)在 `app/src/bin/openwarp_oss.rs` 里是 `dev.openwarp.OpenWarp`,
...
# Inno Setup `AppId` 决定注册表 Uninstall 条目与升级跟踪键。OSS 下固定为 `openwarp-oss`,
# 避免留在默认的 `warp-terminal-oss` 上。其他 channel 走 .iss 里的默认
# `warp-terminal-{ReleaseChannel}`。
if ("$CHANNEL" -eq 'oss') {
    $INNO_APP_ID = 'openwarp-oss'
} else {
