...
impl OrchestrationEventStreamer {
    ...
    fn finish_remote_child_placeholder(
        ...
        let Some(terminal_surface_id) = BlocklistAIHistoryModel::as_ref(ctx)
            .terminal_view_id_for_conversation(&parent_conversation_id)
        else {
            log::warn!(
    ...
    fn should_skip_sse_for_dormant_local_claude_child(
        ...
        conversation.is_child_agent_conversation()
            ...
            // Upstream also consults `ServerAIConversationMetadata.harness`;
            // this fork has no server conversation metadata, so the
            // conversation's own orchestration harness stands in for it.
            && (conversation
