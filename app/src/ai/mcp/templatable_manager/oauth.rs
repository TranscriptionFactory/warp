...
pub async fn make_authenticated_client(
    ...
    if is_headless {
        ...
        return Err(AuthError::AuthorizationFailed(
            "MCP server requires OAuth authentication. Please authenticate this server in the \
             OpenWarp desktop app first, then try again."
                .to_string(),
        ));
    }
    ...
    oauth_state
        .start_authorization(&[], &redirect_uri, Some("OpenWarp"))
        .await?;
    ...
    let config = match auth_manager.register_client("OpenWarp", &redirect_uri).await {
        Ok(config) => config,
