...
common-skip = 跳过
common-get-warping = 开始使用 OpenWarp
common-try-again = 重试
common-settings = 设置
common-premium = 高级版
...
agent-management-source-local-agent = OpenWarp（本地智能体）
agent-management-source-cloud-agent = OpenWarp（本地智能体）
...
workspace-menu-update-warp-manually = 手动更新 OpenWarp
...
workspace-menu-view-warp-logs = 查看 OpenWarp 日志
...
app-menu-debug = 调试
app-menu-set-default-terminal = 将 OpenWarp 设为默认终端
app-menu-file = 文件
app-menu-edit = 编辑
app-menu-use-warp-prompt = 使用 OpenWarp 提示符
...
app-menu-show-warpified-ssh-blocks = 显示 OpenWarp 化 SSH 块
app-menu-hide-warpified-ssh-blocks = 隐藏 OpenWarp 化 SSH 块
...
app-menu-help = 帮助
app-menu-warp-documentation = OpenWarp 文档...
app-menu-github-issues = GitHub Issues...
app-menu-warp-slack-community = OpenWarp Slack 社区...
workspace-update-and-relaunch-warp = 更新并重启 OpenWarp
workspace-updating-to-version = 正在更新到（{ $version }）
workspace-update-warp-manually = 手动更新 OpenWarp
...
terminal-banner-shell-config-incompatible = 你的 shell 配置与 OpenWarp 不兼容...{"  "}
...
ssh-remote-choice-install-extension = 安装 OpenWarp 的 SSH 扩展
...
ssh-remote-choice-continue-without-installing-desc = 你仍可获得 OpenWarp 化体验，但不包含智能体功能。
...
ai-document-save-and-sync-tooltip = 保存并将此计划自动同步到你的 OpenWarp Drive
ai-document-show-in-warp-drive = 在 OpenWarp Drive 中查看
...
ai-footer-enable-notifications-tooltip = 安装 Warp 插件以在 OpenWarp 中启用丰富的智能体通知
...
zap-launch-title = OpenWarp 现已开源
zap-launch-description = 你和我们的社区现在可以使用智能体优先的工作流参与构建 OpenWarp。
zap-launch-contribute-title = 参与贡献
zap-launch-contribute-description = OpenWarp 客户端代码现已开源。你可以先使用 /feedback skill 创建 issue，并阅读这里的贡献指南。
zap-launch-contribute-link-text = 这里
zap-launch-oad-title = 开放自动化开发
zap-launch-oad-description = OpenWarp 仓库由智能体优先的本地工作流管理，并由 Oz 本地智能体体验提供支持。
...
hoa-finish = 完成
session-config-get-warping = 开始使用 OpenWarp
...
terminal-warpify-subshell-tooltip = 在此会话中启用 OpenWarp shell 集成
terminal-use-agent = 使用智能体
terminal-use-agent-tooltip = 让 OpenWarp 智能体协助
terminal-give-control-back-to-agent = 将控制权交还给智能体
terminal-resume-agent-tooltip = 让 OpenWarp 智能体继续
...
terminal-open-in-warp = 在 OpenWarp 中打开
terminal-open-conversation-in-warp-tooltip = 在 OpenWarp 桌面端打开此对话
...
terminal-input-agent-hint-deploy-react-vercel = 让 OpenWarp 处理任何任务，例如将 React 应用部署到 Vercel 并配置环境变量
terminal-input-agent-hint-debug-python-ci = 让 OpenWarp 处理任何任务，例如帮我调试 Python 测试在 CI 中失败的原因
terminal-input-agent-hint-setup-microservice = 让 OpenWarp 处理任何任务，例如用 Docker 搭建新的微服务并创建部署流水线
terminal-input-agent-hint-fix-node-memory-leak = 让 OpenWarp 处理任何任务，例如查找并修复 Node.js 应用中的内存泄漏
terminal-input-agent-hint-backup-postgres = 让 OpenWarp 处理任何任务，例如为 PostgreSQL 数据库创建备份脚本并设置定时任务
terminal-input-agent-hint-migrate-mysql-postgres = 让 OpenWarp 处理任何任务，例如帮我把数据从 MySQL 迁移到 PostgreSQL
terminal-input-agent-hint-monitor-aws = 让 OpenWarp 处理任何任务，例如为 AWS 基础设施设置监控和告警
terminal-input-agent-hint-build-fastapi = 让 OpenWarp 处理任何任务，例如用 FastAPI 为移动应用构建 REST API
terminal-input-agent-hint-optimize-sql = 让 OpenWarp 处理任何任务，例如帮我优化运行缓慢的 SQL 查询
terminal-input-agent-hint-github-actions = 让 OpenWarp 处理任何任务，例如创建 GitHub Actions 工作流，在合并后自动部署
terminal-input-agent-hint-redis-cache = 让 OpenWarp 处理任何任务，例如为 Web 应用设置 Redis 缓存
terminal-input-agent-hint-kubernetes-pods = 让 OpenWarp 处理任何任务，例如帮我排查 Kubernetes Pod 持续崩溃的原因
terminal-input-agent-hint-bigquery-pipeline = 让 OpenWarp 处理任何任务，例如构建数据流水线来处理 CSV 文件并加载到 BigQuery
terminal-input-agent-hint-ssl-https = 让 OpenWarp 处理任何任务，例如设置 SSL 证书并为域名配置 HTTPS
terminal-input-agent-hint-refactor-legacy-code = 让 OpenWarp 处理任何任务，例如帮我将这段遗留代码重构为现代设计模式
terminal-input-agent-hint-unit-tests = 让 OpenWarp 处理任何任务，例如为认证服务创建单元测试
terminal-input-agent-hint-elk-logs = 让 OpenWarp 处理任何任务，例如为分布式系统设置 ELK 日志聚合
terminal-input-agent-hint-oauth-express = 让 OpenWarp 处理任何任务，例如帮我在 Express.js 应用中实现 OAuth2 认证
terminal-input-agent-hint-optimize-docker = 让 OpenWarp 处理任何任务，例如优化 Docker 镜像以减少构建时间和体积
terminal-input-agent-hint-ab-testing = 让 OpenWarp 处理任何任务，例如为 Web 应用搭建 A/B 测试基础设施
...
terminal-model-specs-description = OpenWarp 对模型在执行器中的表现、额度消耗速率和任务速度的基准测试。
...
onboarding-callout-meet-input-title = 认识 OpenWarp 输入框
...
onboarding-callout-nl-support-text-prefix = 默认情况下自然语言输入处于关闭状态。启用后，你可以直接用自然语言输入请求，OpenWarp 会为智能体自动识别查询。你也始终可以使用
onboarding-callout-enable-nl-detection = 启用自然语言识别
onboarding-callout-new-agent-title = 认识 OpenWarp 全新的智能体体验
...
language-widget-secondary = 重启 OpenWarp 以让此更改完全生效。
language-restart-required-title = 语言已切换
language-restart-required-body = OpenWarp 的界面语言已更新。部分文字会立即切换，完整生效需要重启 OpenWarp。
...
settings-section-warp-agent = OpenWarp 智能体
...
# about_page.rs
settings-about-copyright = 版权所有 2026 OpenWarp
settings-about-automatic-updates-label = 自动检查更新
settings-about-automatic-updates-description = 开启后，OpenWarp 会在后台定期检查是否有新版本;发现新版本后自动下载安装包到本地缓存,等您主动点击「立即安装」时才会启动安装程序,期间不会改动当前正在运行的 OpenWarp。
...
settings-about-update-install-hint-macos = 安装包将打开,请把 OpenWarp 拖到「应用程序」文件夹完成升级。
settings-about-update-install-hint-windows = 安装向导将启动,按提示完成升级。
settings-about-update-install-hint-linux = AppImage 将就地更新并重启 OpenWarp。
...
# ---- list_page.rs ----
settings-mcp-list-description = 添加 MCP 服务器以扩展 OpenWarp Agent 的能力。MCP 服务器通过标准化接口向 agent 暴露数据源或工具，本质上类似插件。你可以添加自定义服务器，或使用预设快速开始使用流行的服务器。
...
settings-mcp-list-section-shared-by-warp-and-team = 来自 OpenWarp 和 { $name }
settings-mcp-list-section-shared-by-warp-and-other-devices = 由 OpenWarp 和其他设备共享
settings-mcp-list-section-shared-from-warp = 来自 OpenWarp 的共享
...
settings-platform-description = 创建并管理 API 密钥，允许本地 agent 访问你的 OpenWarp 账户。
    了解更多请访问
...
settings-platform-zero-state-description = 创建密钥以管理对 OpenWarp 的外部访问
settings-platform-create-api-key-description-personal = 此 API 密钥与你的用户绑定，可用于向你的 OpenWarp 账户发起请求。
settings-platform-create-api-key-description-team = 此 API 密钥与你的团队绑定，可用于代表你的团队发起请求。
settings-platform-create-api-key-name-placeholder = OpenWarp API 密钥
...
settings-referrals-page-title = 邀请朋友加入 OpenWarp
...
settings-referrals-reward-intro = 推荐朋友即可获得 OpenWarp 专属周边*
...
settings-warpify-description-prefix = 配置 OpenWarp 是否尝试对特定 Shell 执行 "Warpify"（为其添加命令块、输入模式等支持）。
...
settings-warpify-install-ssh-extension-description = 控制远程主机未安装 OpenWarp 的 SSH 扩展时的安装行为。
...
# 章节 / 副标题
settings-ai-warp-agent-header = OpenWarp 智能体
...
settings-ai-command-denylist-description = 匹配命令的正则表达式，OpenWarp 智能体执行这些命令前必须征得许可。
settings-ai-command-allowlist = 命令允许列表
settings-ai-command-allowlist-description = 匹配命令的正则表达式，OpenWarp 智能体可自动执行这些命令。
...
settings-ai-mcp-allowlist-description = 允许 OpenWarp 智能体调用这些 MCP 服务器。
settings-ai-mcp-denylist = MCP 拒绝列表
settings-ai-mcp-denylist-description = OpenWarp 智能体调用此列表中的任何 MCP 服务器前都必须征得许可。
settings-ai-info-banner-managed-by-workspace = 你的部分权限由工作区管理。

# 模型 / 配置
settings-ai-base-model = 基础模型
settings-ai-base-model-description = 此模型作为 OpenWarp 智能体背后的主要引擎，驱动大部分交互，并在需要时调用其他模型完成规划或代码生成等任务。OpenWarp 可能根据模型可用性自动切换备用模型，或将其用于会话摘要等辅助任务。
settings-ai-show-model-picker-in-prompt = 在提示中显示模型选择器
settings-ai-codebase-context = 代码库上下文
settings-ai-codebase-context-description = 允许 OpenWarp 智能体生成代码库的概要作为上下文。代码从不存储到我们的服务器。
...
settings-ai-rules-description = 规则帮助 OpenWarp 智能体遵循你的约定，无论是针对代码库还是特定工作流。
...
settings-ai-cloud-agent-computer-use-description = 在 OpenWarp 应用中启动的智能体会话中启用计算机使用。

# AWS Bedrock
settings-ai-aws-bedrock-toggle = 使用 AWS Bedrock 凭证
settings-ai-aws-bedrock-description = OpenWarp 加载并发送本地 AWS CLI 凭证以使用 Bedrock 支持的模型。
settings-ai-aws-bedrock-description-managed = OpenWarp 加载并发送本地 AWS CLI 凭证以使用 Bedrock 支持的模型。此设置由你的组织管理。
...
settings-features-make-default-terminal = 将 OpenWarp 设为默认终端
...
settings-features-default-terminal-current = OpenWarp 已是默认终端
...
settings-features-restart-warp-to-apply = 重启 OpenWarp 以使更改生效。
...
settings-exec-profile-editor-base-model-desc = 该模型作为智能体的主要引擎，驱动绝大多数交互，并在需要时调用其他模型完成规划或代码生成等任务。OpenWarp 可能基于模型可用性或辅助任务（如对话摘要）自动切换到备选模型。
...
settings-show-blocks-confirm-dialog-text = 确定要取消共享此命令块吗？
    ...
    取消后将无法通过链接访问，并将从 OpenWarp 服务器永久删除。
...
settings-appearance-custom-icon-restart-warning = 你可能需要重启 OpenWarp 才能让 macOS 应用所选图标样式。
...
settings-appearance-input-mode-pinned-bottom = 固定在底部（OpenWarp 模式）
...
settings-environments-page-description = 环境定义了你的 ambient agent 在哪里运行。可通过 GitHub（推荐）、OpenWarp 辅助配置或手动配置在几分钟内创建一个。
...
settings-environments-section-team-default = 由 OpenWarp 和本地设备提供
settings-environments-section-team-named = 由 OpenWarp 和 { $team } 共享
...
quit-warning-title-app = 退出 OpenWarp？
...
keybinding-desc-workspace-left-panel-warp-drive = 左侧面板：OpenWarp Drive
...
keybinding-desc-workspace-toggle-warp-drive = 切换 OpenWarp Drive
...
keybinding-desc-workspace-search-drive = 搜索 OpenWarp Drive
...
keybinding-desc-workspace-terminate-app = 退出 OpenWarp
...
keybinding-desc-workspace-export-all-warp-drive-objects = 导出所有 OpenWarp Drive 对象
keybinding-desc-workspace-install-cli = 安装 Oz CLI 命令
keybinding-desc-workspace-uninstall-cli = 卸载 Oz CLI 命令

# AI 助手 / agent
keybinding-desc-workspace-toggle-ai-assistant = 切换 OpenWarp AI
...
keybinding-desc-workspace-show-settings-about-menu = 关于 OpenWarp
...
keybinding-desc-workspace-view-logs = 查看 OpenWarp 日志
...
keybinding-desc-workspace-init-project = 为 OpenWarp 初始化项目
...
keybinding-desc-workspace-open-zap-launch-modal = [调试] 打开 OpenWarp 启动弹窗
keybinding-desc-workspace-reset-zap-launch-modal-state = [调试] 重置 OpenWarp 启动弹窗状态
...
# 终端输入相关 binding
keybinding-desc-input-show-network-log = 显示 OpenWarp 网络日志
...
keybinding-desc-ai-assistant-close = 关闭 OpenWarp AI
keybinding-desc-ai-assistant-focus-terminal-input = 从 OpenWarp AI 切回终端输入
keybinding-desc-ai-assistant-restart = 重启 OpenWarp AI
...
keybinding-desc-terminal-ask-ai-selection = 就所选内容询问 OpenWarp AI
keybinding-desc-terminal-ask-ai-last-block = 就最近的命令块询问 OpenWarp AI
keybinding-desc-terminal-ask-ai = 询问 OpenWarp AI
...
toggle-suffix-ssh-wrapper = OpenWarp SSH 包装器
...
settings-external-editor-choose-layout = 选择在 OpenWarp 中打开文件的布局
settings-external-editor-tabbed-header = 多个文件合并到同一编辑器面板
settings-external-editor-tabbed-desc = 开启后，同一标签页中打开的文件会自动归并到单一编辑器面板。
settings-external-editor-prefer-markdown = 默认用 OpenWarp Markdown 查看器打开 Markdown 文件
...
menu-block-open-in-warp = 在 OpenWarp 中打开
...
menu-block-ask-warp-ai = 询问 OpenWarp AI
...
menu-input-ask-warp-ai = 询问 OpenWarp AI
...
slash-cmd-open-skill-desc = 在 OpenWarp 内置编辑器中打开技能的 markdown 文件
slash-cmd-skills-desc = 调用技能
slash-cmd-add-prompt-desc = 添加新的智能体提示词
slash-cmd-add-rule-desc = 为智能体添加新的全局规则
slash-cmd-open-file-desc = 在 OpenWarp 代码编辑器中打开文件
...
prompt-editor-warp-prompt-section = OpenWarp 终端提示词
...
welcome-tips-command-palette-description = 无需双手离开键盘，即可轻松发现 OpenWarp 的全部功能。
...
welcome-tips-theme-picker-description = 选择内置主题，让 OpenWarp 更符合你的风格。也可以创建自己的主题。
...
drive-local-storage-limit-description = 本地存储上限会在此设备上生效。请移除不再需要的项目，为新的 OpenWarp Drive 对象腾出空间。
drive-sign-up = 本地管理
...
workflow-ai-assist-tooltip = 使用 OpenWarp AI 生成标题、描述或参数
...
wasm-nux-open-desktop-title = 在 OpenWarp 桌面版中打开？
wasm-nux-open-desktop-detail = 之后的链接会自动在桌面版中打开。
wasm-nux-open-desktop-confirm = 在 OpenWarp 中打开
wasm-nux-download-title = 下载 OpenWarp 桌面版？
wasm-nux-download-description = OpenWarp 是一款内置 AI 和团队知识的智能终端。
wasm-nux-learn-more = 了解更多
wasm-nux-download-confirm = 下载
wasm-nux-object-kind-drive-objects = OpenWarp Drive 对象
wasm-nux-object-kind-warp-links = OpenWarp 链接
...
auth-override-warning-confirm-title = 要删除个人 OpenWarp Drive 对象和偏好吗？
auth-override-warning-description = 看起来你刚刚通过网页浏览器登录了一个 OpenWarp 账号。如果继续，当前匿名会话中的所有个人 OpenWarp Drive 对象和偏好都将被永久删除。
...
auth-override-warning-accessibility-help = OpenWarp 检测到你通过网页浏览器进行了新的登录。按 Esc 键取消，并继续在未登录状态下使用 OpenWarp。
...
auth-needs-sso-link-detail = 点击下方按钮，将你的 OpenWarp 账号关联到你的 SSO 提供商。
...
auth-offline-first-use-description = 你当前处于离线状态。首次使用 OpenWarp 时需要联网。
auth-offline-first-use-learn-more = 了解更多
auth-offline-overlay-title = 离线使用 OpenWarp
auth-offline-overlay-paragraph-1 = OpenWarp 的本地功能可以离线使用。
auth-offline-overlay-paragraph-2 = 首次使用时可能需要联网完成依赖初始化，之后本地智能体和终端功能不依赖 OpenWarp 云服务。
...
auth-privacy-settings-help-improve = 帮助改进 OpenWarp
auth-privacy-settings-help-improve-description = 高层级的功能使用数据可以帮助 OpenWarp 产品团队确定路线图优先级。
auth-privacy-settings-learn-more = 了解更多
auth-privacy-settings-send-crash-reports = 发送崩溃报告
auth-privacy-settings-crash-reports-description = 崩溃报告能帮助 OpenWarp 工程团队了解稳定性情况并改进性能。
...
auth-logout-unsynced-drive-objects-warning = 你有 { $count } 个未同步的 OpenWarp Drive 对象。退出登录后将失去这些对象。
...
cli-agent-plugin-codex-install-title = 为 Codex 启用 OpenWarp 通知
cli-agent-plugin-codex-install-subtitle = 将 Codex 更新到最新版本，然后启用聚焦时通知，这样你工作时 OpenWarp 就能显示通知。
...
cli-agent-plugin-deepseek-install-title = 为 DeepSeek 启用 OpenWarp 通知
...
terminal-bootstrapping-installing-warp-ssh-extension-progress = 正在安装 OpenWarp SSH 扩展...（{ $p }%）
terminal-bootstrapping-installing-warp-ssh-extension = 正在安装 OpenWarp SSH 扩展...
terminal-bootstrapping-updating-warp-ssh-extension = 正在更新 OpenWarp SSH 扩展...
...
agent-tip-command-palette = 按 <keybinding> 打开命令面板，访问 OpenWarp 操作和快捷方式。
agent-tip-warp-drive = 将可复用的工作流、Notebook 和提示词存入你的
agent-tip-redirect-running-agent = 输入新的提示词即可在智能体运行时重定向它。
agent-tip-add-context = 输入 `@` 将文件、块或 OpenWarp Drive 对象作为上下文添加到提示词。
...
hoa-welcome-banner-title = 引入通用智能体支持：用 OpenWarp 增强任何编码智能体
...
hoa-feature-native-code-review-description = 将 OpenWarp 代码审查中的行内评论直接发送到 Claude Code、Codex 或 OpenCode
resource-center-whats-new-section = 新变化
resource-center-getting-started-section = 入门
resource-center-maximize-warp-section = 充分使用 OpenWarp
...
resource-center-command-palette-description = 通过键盘访问 OpenWarp 的全部功能。
resource-center-set-theme-title = 设置主题
resource-center-set-theme-description = 选择主题，让 OpenWarp 符合你的偏好。
resource-center-custom-prompt-title = 使用自定义提示符
resource-center-custom-prompt-description = 设置 OpenWarp 以遵循你的 PS1 设置
resource-center-view-documentation = 查看文档
resource-center-integrate-ide-title = 将 OpenWarp 与 IDE 集成
resource-center-integrate-ide-description = 配置 OpenWarp，从你最常用的开发工具中启动
resource-center-how-warp-uses-warp-title = OpenWarp 如何使用 OpenWarp
resource-center-how-warp-uses-warp-description = 了解 OpenWarp 工程团队如何使用他们最喜欢的功能
...
onboarding-intention-title = 欢迎使用 OpenWarp
...
onboarding-ai-feature-warp-agents = OpenWarp 智能体
...
onboarding-agent-title = 自定义你的 OpenWarp 智能体
...
onboarding-agent-disable-warp-agent = 禁用 OpenWarp 智能体
...
onboarding-project-subtitle = 设置项目，让 OpenWarp 更适合编码。
...
onboarding-customize-title = 自定义你的 OpenWarp
...
onboarding-free-user-agent-title = 使用 OpenWarp 内置智能体进行开发
onboarding-free-user-agent-description = 使用 Oz（OpenWarp 内置智能体）迭代、规划和构建。本地运行，不依赖 OpenWarp 云服务。
...
onboarding-free-user-subscribe-title = 订阅后即可使用 OpenWarp 的智能体开发能力。
...
onboarding-free-user-subscribe-item-drive = 不限量 OpenWarp Drive 对象和协作
...
auth-login-slide-title-warp-drive = 开始使用 OpenWarp Drive
...
auth-disable-warp-drive = 停用 OpenWarp Drive
auth-disable-ai-features = 停用 AI 功能
auth-enable-warp-drive = 启用 OpenWarp Drive
...
auth-disable-warp-drive-confirm-title = 确定要停用 OpenWarp Drive 吗？
auth-disable-ai-features-confirm-title = 确定要停用 AI 功能吗？
auth-disable-warp-drive-confirm-body = OpenWarp Drive 可让你跨设备保存工作流和知识，并与团队分享。继续后，你将无法使用以下功能：
auth-disable-ai-features-confirm-body = 开启 AI 后 OpenWarp 会更强大。继续后，你将无法使用以下功能：
...
auth-require-login-ai-collaboration = 本地 AI 功能不需要 OpenWarp 账户。
auth-require-login-drive-limit = OpenWarp 会将 OpenWarp Drive 对象保存在本机。
auth-require-login-share = 本地 OpenWarp 构建不提供分享功能。
auth-welcome-title = 欢迎使用 OpenWarp！
...
get-started-welcome-title = 欢迎使用 OpenWarp
...
terminal-banner-p10k-supported = Powerlevel10k 现在支持 OpenWarp！{"  "}
...
terminal-banner-pure-unsupported = OpenWarp 尚不支持 Pure。你可以考虑改用受支持的提示符。{"  "}
...
settings-import-reset-to-warp-defaults = 重置为 OpenWarp 默认设置
...
workspace-notification-permission-denied-toast = OpenWarp 没有发送桌面通知的权限。
workspace-troubleshoot-notifications-link = 排查通知问题
workspace-plan-synced-to-warp-drive-toast = 计划已同步到你的 OpenWarp Drive
workspace-remote-control-link-copied-toast = 远程控制链接已复制。
workspace-update-now = 立即更新
workspace-update-warp = 更新 OpenWarp
...
workspace-version-deprecation-without-permissions-banner = 若不立即更新，部分 OpenWarp 功能可能无法正常工作，但 OpenWarp 无法执行更新。
workspace-new-version-unable-to-update-banner = 有新版本可用，但 OpenWarp 无法执行更新。
workspace-unable-to-launch-new-installed-version = OpenWarp 无法启动新安装的版本。
...
terminal-ssh-report-issue-prefix = 我们正在积极改进 OpenWarp 中 SSH 的稳定性。请考虑
...
terminal-agent-mode-setup-title = 为此代码库优化 OpenWarp？
...
notifications-banner-permissions-denied-title = OpenWarp 被拒绝发送通知的权限。
...
tooltip-secrets-not-sent-to-warp-server = *密钥不会发送到 OpenWarp 服务器。
