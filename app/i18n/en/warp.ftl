# OpenWarp Desktop — English (source-of-truth locale)
...
agent-management-source-local-agent = OpenWarp (local agent)
agent-management-source-cloud-agent = OpenWarp agent
...
workspace-menu-update-warp-manually = Update OpenWarp manually
...
workspace-menu-view-warp-logs = View OpenWarp logs
...
app-menu-warp = OpenWarp
app-menu-preferences = Preferences
app-menu-privacy-policy = Privacy Policy...
app-menu-debug = Debug
app-menu-set-default-terminal = Set OpenWarp as Default Terminal
app-menu-file = File
app-menu-edit = Edit
app-menu-use-warp-prompt = Use OpenWarp's Prompt
...
app-menu-help = Help
app-menu-warp-documentation = OpenWarp Documentation...
app-menu-github-issues = GitHub Issues...
app-menu-warp-slack-community = OpenWarp Slack Community...
workspace-update-and-relaunch-warp = Update and relaunch OpenWarp
workspace-updating-to-version = Updating to ({ $version })
workspace-update-warp-manually = Update OpenWarp manually
...
terminal-banner-shell-config-incompatible = Your shell configuration is incompatible with OpenWarp...{"  "}
...
ssh-remote-choice-install-extension = Install OpenWarp's SSH extension
ssh-remote-choice-install-extension-desc = Install OpenWarp's extension to enable agent features like file browsing, code review, and intelligent command completions in this session.
...
ai-document-save-and-sync-tooltip = Save and auto-sync this plan to your OpenWarp Drive
ai-document-show-in-warp-drive = Show in OpenWarp Drive
...
ai-footer-enable-notifications-tooltip = Install the Warp plugin to enable rich agent notifications within OpenWarp
...
zap-launch-title = OpenWarp is now open-source
zap-launch-description = You, our community, can participate in building OpenWarp using an agent-first workflow.
zap-launch-contribute-title = Contribute
zap-launch-contribute-description = OpenWarp's client code is now open source. Get started by using the /feedback skill to open an issue, and follow the contribution guidelines here.
zap-launch-contribute-link-text = here
zap-launch-oad-title = Open Automated Development
zap-launch-oad-description = The OpenWarp repo is managed by an agent-first local workflow powered by Oz.
...
terminal-warpify-subshell-tooltip = Enable OpenWarp shell integration in this session
terminal-use-agent = Use agent
terminal-use-agent-tooltip = Ask the OpenWarp agent to assist
terminal-give-control-back-to-agent = Give control back to agent
terminal-resume-agent-tooltip = Ask the OpenWarp agent to resume
...
terminal-open-in-warp = Open in OpenWarp
terminal-open-conversation-in-warp-tooltip = Open this conversation in the OpenWarp desktop app
...
terminal-input-agent-hint-deploy-react-vercel = OpenWarp anything e.g. Deploy my React app to Vercel and set up environment variables
terminal-input-agent-hint-debug-python-ci = OpenWarp anything e.g. Help me debug why my Python tests are failing in CI
terminal-input-agent-hint-setup-microservice = OpenWarp anything e.g. Set up a new microservice with Docker and create the deployment pipeline
terminal-input-agent-hint-fix-node-memory-leak = OpenWarp anything e.g. Find and fix the memory leak in my Node.js application
terminal-input-agent-hint-backup-postgres = OpenWarp anything e.g. Create a backup script for my PostgreSQL database and schedule it
terminal-input-agent-hint-migrate-mysql-postgres = OpenWarp anything e.g. Help me migrate my data from MySQL to PostgreSQL
terminal-input-agent-hint-monitor-aws = OpenWarp anything e.g. Set up monitoring and alerts for my AWS infrastructure
terminal-input-agent-hint-build-fastapi = OpenWarp anything e.g. Build a REST API for my mobile app using FastAPI
terminal-input-agent-hint-optimize-sql = OpenWarp anything e.g. Help me optimize my SQL queries that are running slowly
terminal-input-agent-hint-github-actions = OpenWarp anything e.g. Create a GitHub Actions workflow to automatically deploy on merge
terminal-input-agent-hint-redis-cache = OpenWarp anything e.g. Set up Redis caching for my web application
terminal-input-agent-hint-kubernetes-pods = OpenWarp anything e.g. Help me troubleshoot why my Kubernetes pods keep crashing
terminal-input-agent-hint-bigquery-pipeline = OpenWarp anything e.g. Build a data pipeline to process CSV files and load them into BigQuery
terminal-input-agent-hint-ssl-https = OpenWarp anything e.g. Set up SSL certificates and configure HTTPS for my domain
terminal-input-agent-hint-refactor-legacy-code = OpenWarp anything e.g. Help me refactor this legacy code to use modern design patterns
terminal-input-agent-hint-unit-tests = OpenWarp anything e.g. Create unit tests for my authentication service
terminal-input-agent-hint-elk-logs = OpenWarp anything e.g. Set up log aggregation with ELK stack for my distributed system
terminal-input-agent-hint-oauth-express = OpenWarp anything e.g. Help me implement OAuth2 authentication in my Express.js app
terminal-input-agent-hint-optimize-docker = OpenWarp anything e.g. Optimize my Docker images to reduce build times and size
terminal-input-agent-hint-ab-testing = OpenWarp anything e.g. Set up A/B testing infrastructure for my web application
...
terminal-model-specs-description = OpenWarp's benchmarks for how well a model performs in our harness, the rate at which it consumes credits, and task speed.
...
onboarding-callout-meet-input-title = Meet the OpenWarp input
...
onboarding-callout-nl-support-text-prefix = Natural language input is off by default. If enabled, you can type requests in plain English and OpenWarp will autodetect queries for the agent. You can always override them using
onboarding-callout-enable-nl-detection = Enable Natural Language Detection
onboarding-callout-new-agent-title = Introducing OpenWarp's new agent experience
...
language-widget-secondary = Restart OpenWarp for the change to fully take effect.
language-restart-required-title = Language changed
language-restart-required-body = OpenWarp's UI language has been updated. Some text will switch immediately, but a full restart is required for the change to take effect everywhere.
...
settings-section-warp-agent = OpenWarp Agent
...
# about_page.rs
settings-about-copyright = Copyright 2026 OpenWarp
settings-about-automatic-updates-label = Automatic updates
settings-about-automatic-updates-description = When enabled, OpenWarp checks for new versions in the background and downloads the installer to a local cache. The currently running OpenWarp is not touched until you click "Install now" to launch the installer yourself.
...
settings-about-update-install-hint-macos = The installer will open — drag OpenWarp into your Applications folder to finish.
settings-about-update-install-hint-windows = The setup wizard will launch — follow the prompts to finish the upgrade.
settings-about-update-install-hint-linux = The AppImage will be replaced in place and OpenWarp will restart.
...
# ---- list_page.rs ----
settings-mcp-list-description = Add MCP servers to extend the OpenWarp Agent's capabilities. MCP servers expose data sources or tools to agents through a standardized interface, essentially acting like plugins. Add a custom server, or use the presets to get started with popular servers.
...
settings-mcp-list-section-shared-by-warp-and-team = Available from OpenWarp and { $name }
settings-mcp-list-section-shared-by-warp-and-other-devices = Shared by OpenWarp and from other devices
settings-mcp-list-section-shared-from-warp = Shared from OpenWarp
...
settings-platform-description = Create and manage API keys to allow local agents to access your OpenWarp account.
...
settings-platform-zero-state-description = Create a key to manage external access to OpenWarp
settings-platform-create-api-key-description-personal = This API key is tied to your user and can make requests against your OpenWarp account.
...
settings-platform-create-api-key-name-placeholder = OpenWarp API Key
...
settings-referrals-page-title = Invite a friend to OpenWarp
...
settings-referrals-reward-intro = Get exclusive OpenWarp goodies when you refer someone*
...
settings-warpify-description-prefix = Configure whether OpenWarp attempts to "Warpify" (add support for blocks, input modes, etc) certain shells.
...
settings-warpify-install-ssh-extension-description = Controls the installation behavior for OpenWarp's SSH extension when a remote host doesn't have it installed.
...
# Section / sub-headers
settings-ai-warp-agent-header = OpenWarp Agent
...
settings-ai-command-denylist-description = Regular expressions to match commands that the OpenWarp Agent should always ask permission to execute.
settings-ai-command-allowlist = Command allowlist
settings-ai-command-allowlist-description = Regular expressions to match commands that can be automatically executed by the OpenWarp Agent.
...
settings-ai-mcp-allowlist-description = Allow the OpenWarp Agent to call these MCP servers.
settings-ai-mcp-denylist = MCP denylist
settings-ai-mcp-denylist-description = The OpenWarp Agent will always ask for permission before calling any MCP servers on this list.
...
settings-ai-base-model-description = This model serves as the primary engine behind the OpenWarp Agent. It powers most interactions and invokes other models for tasks like planning or code generation when necessary. OpenWarp may automatically switch to alternate models based on model availability or for auxiliary tasks such as conversation summarization.
settings-ai-show-model-picker-in-prompt = Show model picker in prompt
settings-ai-codebase-context = Codebase Context
settings-ai-codebase-context-description = Allow the OpenWarp Agent to generate an outline of your codebase that can be used for context. No code is ever stored on our servers.
...
settings-ai-rules-description = Rules help the OpenWarp Agent follow your conventions, whether for codebases or specific workflows.
...
settings-ai-cloud-agent-computer-use-description = Enable computer use in agent conversations started from the OpenWarp app.

# AWS Bedrock
settings-ai-aws-bedrock-toggle = Use AWS Bedrock credentials
settings-ai-aws-bedrock-description = OpenWarp loads and sends local AWS CLI credentials for Bedrock-supported models.
settings-ai-aws-bedrock-description-managed = OpenWarp loads and sends local AWS CLI credentials for Bedrock-supported models. This setting is managed by your organization.
...
settings-features-make-default-terminal = Make OpenWarp the default terminal
...
settings-features-default-terminal-current = OpenWarp is the default terminal
...
settings-features-restart-warp-to-apply = Restart OpenWarp for changes to take effect.
...
settings-exec-profile-editor-base-model-desc = This model serves as the primary engine behind the agent. It powers most interactions and invokes other models for tasks like planning or code generation when necessary. OpenWarp may automatically switch to alternate models based on model availability or for auxiliary tasks such as conversation summarization.
...
settings-show-blocks-confirm-dialog-text = Are you sure you want to unshare this block?
    ...
    It will no longer be accessible by link and will be permanently deleted from OpenWarp servers.
...
settings-appearance-custom-icon-restart-warning = You may need to restart OpenWarp for MacOS to apply the preferred icon style.
...
settings-appearance-input-type-warp = OpenWarp
settings-appearance-input-type-shell = Shell (PS1)
settings-appearance-input-position-label = Input position
settings-appearance-input-mode-pinned-bottom = Pin to the bottom (OpenWarp mode)
...
settings-environments-page-description = Environments define where your ambient agents run. Set one up in minutes via GitHub (recommended), OpenWarp-assisted setup, or manual configuration.
...
settings-environments-section-team-default = Provided by OpenWarp and this device
settings-environments-section-team-named = Shared by OpenWarp and { $team }
...
quit-warning-title-app = Quit OpenWarp?
...
# Manage Rules 页面(OpenWarp Drive 中的 AI Fact Collection)。
...
keybinding-desc-workspace-left-panel-warp-drive = Left Panel: OpenWarp Drive
...
keybinding-desc-workspace-toggle-warp-drive = Toggle OpenWarp Drive
keybinding-desc-workspace-toggle-warp-drive-menu = OpenWarp Drive
...
keybinding-desc-workspace-search-drive = Search OpenWarp Drive
...
keybinding-desc-workspace-terminate-app = Quit OpenWarp
...
keybinding-desc-workspace-export-all-warp-drive-objects = Export all OpenWarp Drive objects
...
# AI assistant / agents
keybinding-desc-workspace-toggle-ai-assistant = Toggle OpenWarp AI
...
keybinding-desc-workspace-show-settings-about-menu = About OpenWarp
...
keybinding-desc-workspace-view-logs = View OpenWarp logs
...
keybinding-desc-workspace-open-zap-launch-modal = [Debug] Open OpenWarp Launch Modal
keybinding-desc-workspace-reset-zap-launch-modal-state = [Debug] Reset OpenWarp Launch Modal State
...
keybinding-desc-input-show-network-log = Show OpenWarp network log
...
keybinding-desc-ai-assistant-close = Close OpenWarp AI
keybinding-desc-ai-assistant-focus-terminal-input = Focus Terminal Input From OpenWarp AI
keybinding-desc-ai-assistant-restart = Restart OpenWarp AI
...
keybinding-desc-terminal-ask-ai-selection = Ask OpenWarp AI about Selection
keybinding-desc-terminal-ask-ai-last-block = Ask OpenWarp AI about last block
keybinding-desc-terminal-ask-ai = Ask OpenWarp AI
...
toggle-suffix-ssh-wrapper = OpenWarp SSH wrapper
...
settings-external-editor-choose-layout = Choose a layout to open files in OpenWarp
...
settings-external-editor-prefer-markdown = Open Markdown files in OpenWarp's Markdown Viewer by default
...
menu-block-open-in-warp = Open in OpenWarp
...
menu-block-ask-warp-ai = Ask OpenWarp AI
...
menu-input-ask-warp-ai = Ask OpenWarp AI
...
slash-cmd-open-skill-desc = Open a skill's markdown file in OpenWarp's built-in editor
...
slash-cmd-open-file-desc = Open a file in OpenWarp's code editor
...
prompt-editor-warp-prompt-section = OpenWarp terminal prompt
...
welcome-tips-command-palette-description = Easily discover everything you can do in OpenWarp without your hands leaving the keyboard.
...
welcome-tips-theme-picker-description = Make OpenWarp your own by choosing a built-in theme. Or create your own.
...
search-filter-display-drive = OpenWarp Drive
...
drive-local-storage-limit-description = Local storage limits are enforced on this device. Remove unused items to create space for new OpenWarp Drive objects.
...
workflow-ai-assist-tooltip = Generate a title, descriptions, or parameters with OpenWarp AI
...
workspace-left-panel-warp-drive = OpenWarp Drive
...
wasm-nux-open-desktop-title = Open in OpenWarp Desktop?
wasm-nux-open-desktop-detail = Future links will automatically open on desktop.
wasm-nux-open-desktop-confirm = Open in OpenWarp
wasm-nux-download-title = Download OpenWarp Desktop?
wasm-nux-download-description = OpenWarp is the intelligent terminal with AI and your dev team's knowledge built-in.
wasm-nux-learn-more = Learn more
wasm-nux-download-confirm = Download
wasm-nux-object-kind-drive-objects = OpenWarp Drive objects
wasm-nux-object-kind-warp-links = OpenWarp links
...
auth-override-warning-confirm-title = Delete personal OpenWarp Drive objects and preferences?
auth-override-warning-description = It looks like you logged into a OpenWarp account through a web browser. If you continue, any personal OpenWarp Drive objects and preferences from this anonymous session will be permanently deleted.
...
auth-override-warning-accessibility-help = OpenWarp has detected a new login from a web browser. Press escape to cancel and continue using OpenWarp without login.
...
auth-needs-sso-link-detail = Click the button below to link your OpenWarp account to your SSO provider.
...
auth-offline-first-use-description = You are currently offline. An internet connection is required to use OpenWarp for the first time.
auth-offline-first-use-learn-more = Learn more
auth-offline-overlay-title = Using OpenWarp Offline
auth-offline-overlay-paragraph-1 = OpenWarp can be used offline for local terminal and agent workflows.
...
auth-privacy-settings-help-improve = Help improve OpenWarp
auth-privacy-settings-help-improve-description = High-level feature usage data helps OpenWarp's product team prioritize the roadmap.
auth-privacy-settings-learn-more = Learn more
auth-privacy-settings-send-crash-reports = Send crash reports
auth-privacy-settings-crash-reports-description = Crash reporting helps OpenWarp's engineering team understand stability and improve performance.
...
auth-logout-unsynced-drive-objects-warning = You have { $count } unsynced OpenWarp Drive { $count ->
        [one] object
       *[other] objects
...
cli-agent-plugin-codex-install-title = Enable OpenWarp Notifications for Codex
cli-agent-plugin-codex-install-subtitle = Update Codex to the latest version, then enable in-focus notifications so OpenWarp can display them while you work.
...
cli-agent-plugin-deepseek-install-title = Enable OpenWarp Notifications for DeepSeek
...
cli-agent-plugin-claude-install-title = Install OpenWarp Plugin for Claude Code
...
cli-agent-plugin-claude-update-title = Update OpenWarp Plugin for Claude Code
...
cli-agent-plugin-gemini-install-title = Install OpenWarp Plugin for Gemini CLI
...
cli-agent-plugin-gemini-update-title = Update OpenWarp Plugin for Gemini CLI
...
cli-agent-plugin-opencode-install-title = Install OpenWarp Plugin for OpenCode
...
cli-agent-plugin-opencode-update-title = Update OpenWarp Plugin for OpenCode
...
terminal-bootstrapping-installing-warp-ssh-extension-progress = Installing OpenWarp SSH Extension... ({ $p }%)
terminal-bootstrapping-installing-warp-ssh-extension = Installing OpenWarp SSH Extension...
terminal-bootstrapping-updating-warp-ssh-extension = Updating OpenWarp SSH Extension...
...
agent-tip-command-palette = <keybinding> to open the Command Palette and access OpenWarp actions and shortcuts.
...
agent-tip-add-context = `@` to add context from files, blocks, or OpenWarp Drive objects to your prompt.
...
agent-tip-action-warp-drive = OpenWarp Drive.
...
hoa-welcome-banner-title = Introducing universal agent support: level up any coding agent with OpenWarp
...
hoa-feature-native-code-review-description = Send inline comments from OpenWarp's code review directly to Claude Code, Codex, or OpenCode
resource-center-whats-new-section = What's New?
resource-center-getting-started-section = Getting Started
resource-center-maximize-warp-section = Maximize OpenWarp
...
resource-center-command-palette-description = Access all of OpenWarp via the keyboard.
resource-center-set-theme-title = Set your theme
resource-center-set-theme-description = Make OpenWarp your own by choosing a theme.
resource-center-custom-prompt-title = Use your custom prompt
resource-center-custom-prompt-description = Set up OpenWarp to honor your PS1 setting
resource-center-view-documentation = View documentation
resource-center-integrate-ide-title = Integrate OpenWarp with your IDE
resource-center-integrate-ide-description = Configure OpenWarp to launch from your most used development tools
resource-center-how-warp-uses-warp-title = How OpenWarp uses OpenWarp
resource-center-how-warp-uses-warp-description = Learn how OpenWarp's engineering team uses their favorite features
...
onboarding-intention-title = Welcome to OpenWarp
...
onboarding-ai-feature-warp-agents = OpenWarp agents
...
onboarding-agent-title = Customize your OpenWarp Agent
...
onboarding-agent-disable-warp-agent = Disable OpenWarp Agent
onboarding-project-title = Open a project
onboarding-project-subtitle = Set up a project to optimize it for coding in OpenWarp.
...
onboarding-customize-title = Customize your OpenWarp
...
onboarding-customize-warp-drive = OpenWarp Drive
...
auth-login-slide-title-warp-drive = Get started with OpenWarp Drive
...
auth-disable-warp-drive = Disable OpenWarp Drive
auth-disable-ai-features = Disable AI features
auth-enable-warp-drive = Enable OpenWarp Drive
...
auth-disable-warp-drive-confirm-title = Are you sure you want to disable OpenWarp Drive?
auth-disable-ai-features-confirm-title = Are you sure you want to disable AI features?
auth-disable-warp-drive-confirm-body = OpenWarp Drive lets you save workflows and knowledge across devices and share them with your team. By continuing, you won't have access to the following features:
auth-disable-ai-features-confirm-body = OpenWarp is better with AI. By continuing, you won't have access to any of the following features:
...
auth-require-login-ai-collaboration = Local AI features do not require a OpenWarp account.
auth-require-login-drive-limit = OpenWarp Drive objects are stored locally in OpenWarp.
auth-require-login-share = Sharing is unavailable in local OpenWarp builds.
auth-welcome-title = Welcome to OpenWarp!
...
get-started-welcome-title = Welcome to OpenWarp
...
terminal-banner-p10k-supported = Powerlevel10k now supports OpenWarp!{"  "}
...
terminal-banner-pure-unsupported = Pure is not yet supported in OpenWarp. You might consider one of the supported prompts as an alternative.{"  "}
...
settings-import-reset-to-warp-defaults = Reset to OpenWarp defaults
...
workspace-notification-permission-denied-toast = OpenWarp doesn't have permission to send desktop notifications.
workspace-troubleshoot-notifications-link = Troubleshoot notifications
workspace-plan-synced-to-warp-drive-toast = Plan synced to your OpenWarp Drive
workspace-remote-control-link-copied-toast = Remote control link copied.
workspace-update-now = Update now
workspace-update-warp = Update OpenWarp
...
workspace-version-deprecation-without-permissions-banner = Some OpenWarp features may not work as expected without updating immediately, but OpenWarp is unable to perform the update.
workspace-new-version-unable-to-update-banner = A new version is available but OpenWarp is unable to perform the update.
workspace-unable-to-launch-new-installed-version = OpenWarp was unable to launch the new installed version.
...
terminal-ssh-report-issue-prefix = We are actively working on improving the stability of SSH in OpenWarp. Please consider{" "}
...
terminal-agent-mode-setup-title = Optimize OpenWarp for this codebase?
...
notifications-banner-permissions-denied-title = OpenWarp was denied permissions to send you notifications.
...
tooltip-secrets-not-sent-to-warp-server = *Secrets are not sent to OpenWarp's server.
