...
fn test_warp_home_skills_and_mcp_paths() {
    let Some(config_dir) = warp_home_config_dir() else {
        panic!("Should be able to compute OpenWarp home config directory");
    };
...
fn test_oss_secure_state_dir_is_disabled() {
    // ChannelState 默认是 Channel::Oss。OpenWarp 不应该探测官方 App Group,
...
fn test_project_path_for_warp_app_id() {
    let project_dirs = project_dirs_for_app_id(AppId::new("dev", "warp", "OpenWarp"), None)
        .expect("should be able to compute project dirs");
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            assert_eq!(project_dirs.project_path(), "dev.warp.OpenWarp");
        } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            assert_eq!(project_dirs.project_path(), "warp-terminal");
        } else if #[cfg(windows)] {
            assert_eq!(project_dirs.project_path(), "warp\\OpenWarp");
        } else {
            unimplemented!("Need to update tests for current platform!");
        }
    }
