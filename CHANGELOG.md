...
- **长任务可靠性**:采集长时间运行命令的进程树活跃信号,避免 Agent 将仍然健康但暂时静默的命令误判为卡死并终止。
- **多 Agent 编排**:加入受 feature flag 保护的 M1 编排基础设施,包括编排事件与拓扑、子 Agent 跟踪、事件流以及本地 Claude Code/Codex 子进程衔接。当前 fork 未配置云端编排接口,因此云端传输保持禁用。
