...
third-party CLI agent, ships a built-in SSH host manager with remote file
browsing and code review, and fixes a number of upstream rendering issues —
...
| Third-party agents | Built-in Warp Agent only | **Any CLI agent — DeepSeek-TUI / Codex / Claude Code / Agy wired in** |
| SSH management | Not built-in | **Built-in SSH host manager (connect / config / tmux)** |
| Remote file browsing | Not built-in | **Built-in SFTP file browser** |
| Remote code review | N/A | **Diff + code review panel on remote SSH hosts** |
| Markdown rendering | Upstream baseline | **Tuned MD pipeline — code blocks, tables, mixed CJK; rendered preview for remote files** |
| Image viewing | Not built-in | **In-app image viewer (local + remote)** |
| Themes | Single global theme | **Per-window themes** |
...
| UI language | English | **English + Simplified Chinese + Japanese, extensible** |
...
- **SFTP file browser** — graphical file browser for remote hosts, browse and
  open remote files as if they were local.
- **Remote code review** — diff viewer and code review panel work across SSH
  sessions. Review changes, stage hunks, and navigate diffs on remote repos
  inside the terminal.
- **Remote Markdown preview** — toggle rendered Markdown preview for files on
  remote hosts, just like local files.
- **In-app image viewer** — open images (PNG, JPEG, GIF, SVG, WebP) directly in
  the terminal. Works on both local and remote files via SFTP.
- **Per-window themes** — set a different theme per window; theme preview applies
...
- **Third-party CLI agents** — bring any CLI agent into the Warp Block model.
  ...
  - **Google Antigravity** (`agy`) — native support
  - **Codex CLI**, **Claude Code**, and other mainstream CLI agents
  - Unified routing through OSC9 / OSC777 into OpenWarp's notification center
...
- **Fully decentralized** — no Warp account, no forced login, no cloud Drive /
  ...
  removed in stages.
- **Markdown rendering improvements** — structured table rendering in AI blocks,
  configurable heading scale, better stability for code blocks, tables, lists
  and mixed Chinese/English text inside AI blocks.
...
- **Enhanced editor support** — Vue file injection highlighting (JS/CSS/TS) and
  block-level comments for Vue SFC files.
- **CLI agent settings UI** — graphical settings panel for configuring CLI agents.
- **Right-aligned user messages** — chat transcript renders user messages on the
  right for clearer conversation flow.
...
- **Third-party CLI agents** — DeepSeek-TUI / Google Antigravity (`agy`) /
  Codex CLI / Claude Code routed through OSC9 into Blocks and the notification
  center
- **SSH host manager** — manage SSH hosts and sessions inside the terminal,
  with tmux integration
- **SFTP file browser** — browse and open remote files in a graphical file tree
- **Remote code review** — diff viewer and review panel on remote SSH repos
- **Remote Markdown preview** — rendered preview toggle for remote files
- **In-app image viewer** — open local and remote images (PNG/JPEG/GIF/SVG/WebP)
  directly in the terminal
- **Per-window themes** — independent theme per window, with non-disruptive preview
...
- **Rendering improvements** — tuned Markdown pipeline (structured tables,
  configurable heading scale) + CJK soft-wrap / bold fixes
...
- **Localized UI** — Simplified Chinese + Japanese + English, community-extensible
- **Built-in themes** — VS Code 2026 Dark and other themes included
- **Vue SFC support** — injection highlighting (JS/CSS/TS) and block-level comments
- **Onkey** — keyboard remapping for custom keybindings
...
3. **Makes remote work native** — SSH / tmux / SFTP / remote diffs / remote
   image viewing are built-in, not bolted on.
...
`TranscriptionFactory/warp` keeps two long-lived branches:

| Branch | Tracks | Purpose |
| --- | --- | --- |
| `main` | `TranscriptionFactory/warp:main` (default) | OpenWarp's main development line. **All PRs target this.** |
...
**Do not click the "Sync fork" button** on `main` in the GitHub web UI. It would merge the entire upstream history straight into OpenWarp's main line and trigger large-scale conflicts. Pull upstream changes through the mirror branch:
...
> **Windows note for DeepSeek-TUI** — DeepSeek-TUI's `[notifications].method` defaults to `auto`,
...
Before filing, please [search existing issues](https://github.com/TranscriptionFactory/warp/issues).
